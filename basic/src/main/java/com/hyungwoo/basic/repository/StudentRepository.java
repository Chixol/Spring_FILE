package com.hyungwoo.basic.repository;
...
// - JPA 기반 REpository를 생성할떄 반드시 상속해야함
...
extends JpaRepository<StudentEntity, Integer>{
    ...
    @Query(
        value = "SELECT s FROM student s WHERE s..studentNumber = ?1AND s.age > ?2", nativeQuery = false
    )
